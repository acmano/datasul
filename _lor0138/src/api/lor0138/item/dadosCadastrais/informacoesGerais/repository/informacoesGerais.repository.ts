...
// Importar repositories de entidades relacionadas
import { FamiliaInformacoesGeraisRepository } from '@api/lor0138/familia/dadosCadastrais/informacoesGerais/repository/informacoesGerais.repository';
import { FamiliaComercialInformacoesGeraisRepository } from '@api/lor0138/familiaComercial/dadosCadastrais/informacoesGerais/repository/informacoesGerais.repository';
import { GrupoDeEstoqueInformacoesGeraisRepository } from '@api/lor0138/grupoDeEstoque/dadosCadastrais/informacoesGerais/repository/informacoesGerais.repository';
import { EstabelecimentoInformacoesGeraisRepository } from '@api/lor0138/estabelecimento/dadosCadastrais/informacoesGerais/repository/informacoesGerais.repository';
...
export class ItemInformacoesGeraisRepository {
  ...
  static async getItemCompleto(itemCodigo: string): Promise<{
    ...
    try {
      ...
      // Helper para validar se código é válido (não null, não undefined, não vazio)
      ...
      const estabelecimentosEnriquecidos = await Promise.all(
        estabelecimentosData.slice(0, 50).map(async (estab) => {
          try {
            ...
            // Se falhar, retorna apenas código
            console.error(`Erro ao buscar estabelecimento ${estab.estabelecimentoCodigo}:`, error);
            return {
              codigo: estab.estabelecimentoCodigo,
