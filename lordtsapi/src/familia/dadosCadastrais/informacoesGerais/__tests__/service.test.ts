...
describe('Service - InformacoesGeraisService (Familia)', () => {
  // ← FIX
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe('getInformacoesGerais - Não Encontrado', () => {
    ...
    it('deve logar quando não encontrado', async () => {
      ...
      expect(log.info).toHaveBeenCalledWith('Família não encontrada', { familiaCodigo: 'F001' });
    });
  });
  ...
  describe('getInformacoesGerais - Erros', () => {
    it('deve re-lançar FamiliaNotFoundError sem conversão', async () => {
      ...
      (FamiliaInformacoesGeraisRepository.getFamiliaMaster as jest.Mock).mockRejectedValue(
        notFoundError
      );

      await expect(InformacoesGeraisService.getInformacoesGerais('F001')).rejects.toThrow(
        FamiliaNotFoundError
      );
    });
    ...
    it('deve converter erros de banco em DatabaseError', async () => {
      ...
      (FamiliaInformacoesGeraisRepository.getFamiliaMaster as jest.Mock).mockRejectedValue(dbError);
  ...
  describe('Integração com Repository', () => {
    it('deve chamar getFamiliaMaster com código correto', async () => {
      ...
      expect(FamiliaInformacoesGeraisRepository.getFamiliaMaster).toHaveBeenCalledWith('F123');
      expect(FamiliaInformacoesGeraisRepository.getFamiliaMaster).toHaveBeenCalledTimes(1);
    });
  });
