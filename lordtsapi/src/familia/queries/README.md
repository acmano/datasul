...
├── listar-todas.sql       # Query para listar todas as famílias
└── get-by-codigo.sql      # Query para buscar família por código
```
...
**Descrição:** Busca todas as famílias cadastradas no sistema
...
**Descrição:** Busca uma família específica pelo código

**Parâmetros:**
- `@familiaCodigo` (varchar) - Código da família
...
const query1 = FamiliaQueries.listarTodas(); // I/O

// Chamadas subsequentes: Retorna do cache
const query2 = FamiliaQueries.listarTodas(); // Memória (rápido!)
```
...
2. Substitua variáveis de teste:
   ```sql
   -- Em vez de @paramfamiliaCodigo
   DECLARE @paramfamiliaCodigo varchar(16) = '001'; -- Valor de teste
   ```
3. Execute no SSMS
