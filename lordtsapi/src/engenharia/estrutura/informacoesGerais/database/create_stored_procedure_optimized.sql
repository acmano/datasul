-- =============================================
-- Script: Criação da Stored Procedure de Estrutura (BOM) - VERSÃO OTIMIZADA
-- Database: SQL Server
-- Descrição: Versão otimizada que processa níveis em lote
--            Reduz de O(n) para O(depth) queries ao Linked Server
-- =============================================

-- Verifica se a SP já existe e a remove
IF OBJECT_ID('dbo.usp_ExplodeEstruturaEProcessos_JSON_v2', 'P') IS NOT NULL
    DROP PROCEDURE dbo.usp_ExplodeEstruturaEProcessos_JSON_v2;
GO

-- Cria a Stored Procedure Otimizada
CREATE PROCEDURE dbo.usp_ExplodeEstruturaEProcessos_JSON_v2
(
...
N'INSERT INTO #root (it_codigo, cod_estabel, desc_item, un)
  ...
  FROM OPENQUERY(' + @ls + N', ''' + REPLACE(@inner_root, '''', '''''') + N''');';
        ...
        INSERT INTO #nivelAtual (it_codigo, cod_estabel, descricao, unidade_medida, nivel, quantidade_estrut, quantidade_acum, parent_linha, data_inicio, data_fim)
        SELECT r.it_codigo, r.cod_estabel, r.desc_item, r.un, 0, NULL, 1, NULL, NULL, NULL
        FROM #root r;

        -- 3) Processar nível por nível (BFS - Breadth-First Search)
        DECLARE @nivelCorrente INT = 0;
        DECLARE @maxIteracoes INT = 50; -- Limite de segurança contra loops infinitos

        WHILE EXISTS (SELECT 1 FROM #nivelAtual) AND @nivelCorrente < @maxIteracoes
        BEGIN
            -- Inserir itens do nível atual em #ttEstrut
            INSERT INTO #ttEstrut (nivel, cod_estabel, it_codigo, es_codigo, descricao, unidade_medida, quantidade_estrut, quantidade_acum, parent_linha, data_inicio, data_fim)
            SELECT
                n.nivel,
                n.cod_estabel,
                n.it_codigo,
            ...
            -- Construir lista de itens para buscar componentes (em lote)
            DECLARE @itemList NVARCHAR(MAX);
            SELECT @itemList = STRING_AGG('''' + REPLACE(it_codigo, '''', '''''') + '''', ',')
            FROM #nivelAtual;

            -- Buscar todos os componentes do nível atual em UMA ÚNICA query
            IF OBJECT_ID('tempdb..#temp_componentes_lote') IS NOT NULL DROP TABLE #temp_componentes_lote;
...
N'SELECT e."it-codigo" AS it_codigo_pai,
  ...
  WHERE e."it-codigo" IN (' + @itemList + N')
    ...
    AND (e."data-termino" IS NULL OR e."data-termino" >= ''' + @data_atual + N''')';
            ...
            SET @sql =
N'INSERT INTO #temp_componentes_lote (it_codigo_pai, es_codigo, qtd_compon, desc_item, un, cod_estabel)
  SELECT it_codigo_pai, es_codigo, qtd_compon, desc_item, un, cod_estabel
  FROM OPENQUERY(' + @ls + N', ''' + REPLACE(@inner_comp_lote, '''', '''''') + N''');';
            EXEC(@sql);

            -- Preparar próximo nível com base nos componentes encontrados
            ...
            DROP TABLE #temp_componentes_lote;

            -- Buscar roteiros e processos para o nível atual (em lote)
            IF OBJECT_ID('tempdb..#temp_roteiros_lote') IS NOT NULL DROP TABLE #temp_roteiros_lote;
            CREATE TABLE #temp_roteiros_lote (
                it_codigo    NVARCHAR(200),
                cod_estabel  NVARCHAR(200),
                cod_roteiro  NVARCHAR(200)
            );

            DECLARE @inner_rot_lote NVARCHAR(MAX) =
N'SELECT p."it-codigo" AS it_codigo,
        p."cod-estabel" AS cod_estabel,
        p."cod-roteiro" AS cod_roteiro
  FROM PUB."proces-item" p
  WHERE p."it-codigo" IN (' + @itemList + N')';

            SET @sql =
N'INSERT INTO #temp_roteiros_lote (it_codigo, cod_estabel, cod_roteiro)
  SELECT it_codigo, cod_estabel, cod_roteiro
  FROM OPENQUERY(' + @ls + N', ''' + REPLACE(@inner_rot_lote, '''', '''''') + N''');';
            BEGIN TRY
                EXEC(@sql);
            END TRY
            BEGIN CATCH
                -- Se não há roteiros, não é erro
            END CATCH

            -- Buscar operações para todos os itens do nível (em lote)
...
N'INSERT INTO #temp_ops_lote (it_codigo, op_codigo, descricao, nr_unidades, numero_homem, tempo_homem, tempo_maquin, proporcao, un_med_tempo, cod_estabel, cc_codigo, gm_codigo, gm_desc, cc_desc, un, cod_roteiro)
  ...
  FROM OPENQUERY(' + @ls + N', ''' + REPLACE(@inner_ops_lote, '''', '''''') + N''');';
            ...
            INNER JOIN #ttEstrut e ON e.it_codigo = p.it_codigo AND e.nivel = @nivelCorrente;
        ...
        BEGIN
            RAISERROR('Aviso: Estrutura atingiu o limite máximo de %d níveis. Possível loop infinito detectado.', 10, 1, @maxIteracoes);
        END
                                ...
                                SELECT
                                    ...
                                    ISNULL(
                                        STUFF((
                                            SELECT ',' + j.json
                                            FROM #json_node j
                                            JOIN #ttEstrut child ON child.linha = j.linha
...
-- GRANT EXECUTE ON dbo.usp_ExplodeEstruturaEProcessos_JSON_v2 TO [seu_usuario];
GO

-- Mensagem de sucesso
PRINT 'Stored procedure otimizada criada com sucesso: dbo.usp_ExplodeEstruturaEProcessos_JSON_v2';
PRINT 'Algoritmo: BFS (Breadth-First Search) com processamento em lote por nível';
PRINT 'Complexidade: O(depth) queries ao invés de O(n)';
GO
