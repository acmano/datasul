-- =============================================
-- Script: Criação da Stored Procedure de Onde Usado (Where Used) - CORRIGIDO
-- Database: SQL Server
-- Descrição: Explode a estrutura de produtos na direção INVERSA
--            Partindo de um componente, sobe até encontrar todos os pais
--            Processamento em lote (BFS) otimizado
-- Correção: STRING_AGG com CAST para NVARCHAR(MAX)
-- =============================================
...
N'INSERT INTO #root (it_codigo, cod_estabel, desc_item, un)
  ...
  FROM OPENQUERY(' + @ls + N', ''' + REPLACE(@inner_root, '''', '''''') + N''');';
        ...
        BEGIN
            ...
            -- ⚠️ CORREÇÃO: STRING_AGG com CAST para NVARCHAR(MAX)
            -- Construir lista de itens para buscar PAIS (em lote)
            ...
            -- Buscar todos os PAIS (onde usado) do nível atual em UMA ÚNICA query
            -- INVERSÃO: WHERE e."es-codigo" IN (...) ao invés de WHERE e."it-codigo" IN (...)
            IF OBJECT_ID('tempdb..#temp_pais_lote') IS NOT NULL DROP TABLE #temp_pais_lote;
            CREATE TABLE #temp_pais_lote (
                it_codigo_filho NVARCHAR(200),  -- Componente (filho)
                it_codigo_pai   NVARCHAR(200),  -- Pai (onde é usado)
...
N'INSERT INTO #temp_pais_lote (it_codigo_filho, it_codigo_pai, qtd_compon, desc_item, un, cod_estabel, data_inicio, data_fim)
  ...
  FROM OPENQUERY(' + @ls + N', ''' + REPLACE(@inner_pais_lote, '''', '''''') + N''');';
            ...
            SELECT
                ...
                @nivelCorrente + 1,
                p.qtd_compon,
                -- Quantidade acumulada: divide ao invés de multiplicar (invertido)
                CASE
...
-- Mensagem de sucesso
PRINT 'Stored procedure CORRIGIDA criada com sucesso: dbo.usp_OndeUsado_JSON';
PRINT 'Correção: STRING_AGG com CAST para NVARCHAR(MAX) para evitar truncamento';
