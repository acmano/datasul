...
N'INSERT INTO #root (it_codigo, cod_estabel, desc_item, un)
  ...
  FROM OPENQUERY(' + @ls + N', ''' + REPLACE(@inner_root, '''', '''''') + N''');';
        ...
        DECLARE @batchSize INT = 200; -- ⚠️ Processar no máximo 50 itens por lote
...
PRINT 'Stored procedure V3 criada com sucesso: dbo.usp_OndeUsado_JSON';
PRINT 'Correções aplicadas:';
PRINT '  - STRING_AGG com CAST para NVARCHAR(MAX)';
PRINT '  - Processamento em LOTES de 200 itens (otimizado para performance)';
PRINT '  - Redução de queries ao Linked Server para evitar timeouts';
