// src/familiaComercial/queries/index.ts
 ...
 *
 * const query = FamiliaComercialQueries.listarTodas(); // Primeira vez: I/O
 * const query2 = FamiliaComercialQueries.listarTodas(); // Subsequente: Cache
 * ```
 */
...
/**
 * Queries disponíveis do módulo FamiliaComercial
 *
 ...
 *    - Conteúdo: SELECT ... FROM pub."fam-comerc"
...
export const FamiliaComercialQueries = {
   ...
   * @example
   * ```typescript
   * const projection = FamiliaComercialQueries.getProjection();
   * const query = `${projection}
   * WHERE familiaComercial."descricao" LIKE '%LINHA BRANCA%'
   * ORDER BY familiaComercial."descricao"`;
   * ```
   */
  getProjection: () => loadQuery('projection.sql'),

  /**
   * Busca uma família comercial específica por código
   *
   ...
   *
   * Parâmetros:
   *  - ? (varchar) - Código da família comercial (parâmetro ODBC)
   * Retorna: FamiliaComercialMasterQueryResult | null
   *
   * Performance:
  ...
  /**
   * Lista todas as famílias comerciais cadastradas
   *
   * Composição: 2 arquivos
   * - projection.sql (SELECT ... FROM)
   ...
   * Retorna: FamiliaComercialListItem[]
   *
   * Performance:
   * - Lê toda a tabela fam-comerc (sem WHERE)
...
export type FamiliaComercialQueriesType = typeof FamiliaComercialQueries;
